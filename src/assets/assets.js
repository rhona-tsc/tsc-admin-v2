import logo from "./logo.png";
import add_icon from "./add_icon.png";
import order_icon from "./order_icon.png";
import upload_area from "./upload_area.png";
import parcel_icon from "./parcel_icon.svg";
import ai_icon from "./ai_icon.png";
import hot_meal_icon from "./hot_meal_icon.png";
import parking_icon from "./parking_icon.png";
import changing_room_icon from "./changing_room_icon.png";
import roaming_icon from "./roaming_icon.png";
import cover_overhead_icon from "./cover_overhead_icon.png";
import dry_and_level_icon from "./dry_and_level_icon.png";
import cross_icon from "./cross_icon.png";
import dropdown_icon from "./dropdown_icon.png";
import info_icon from "./info_icon.png";
import edit_icon from "./edit_icon.png";
import white_add_icon from "./white_add_icon.png";
import white_tick_icon from "./white_tick_icon.png";
import Becky from "./Becky.jpg";
import Callum from "./Callum.jpg";
import Chris from "./Chris.jpg";
import Colin from "./Colin.jpg";
import Cyrus from "./Cyrus.jpg";
import DavidK from "./David-K.jpg";
import David from "./David.jpg";
import Emanuele from "./Emanuele.jpg";
import EmmaMay from "./Emma-May.jpg";
import Eve from "./Eve.jpg";
import Ewan from "./Ewan.jpg";
import Frazer from "./Frazer.jpg";
import Gershom from "./Gershom.jpg";
import Jeremy from "./Jeremy.jpg";
import Joe from "./Joe.jpg";
import Josh from "./Josh.jpg";
import Kedesha from "./Kedesha.jpg";
import Kevin from "./Kevin.jpg";
import Marco from "./Marco.jpg";
import Mariam from "./Mariam.jpg";
import Nicki from "./Nicki.jpg";
import Phoebe from "./Phoebe.jpg";
import Renes from "./Renes.jpg";
import Rya from "./Rya.jpg";
import Sam from "./Sam.jpg";
import Shamyra from "./Shamyra.jpg";
import Sophie from "./Sophie.jpg";
import Torann from "./Torann.jpg";
import Will from "./Will.jpg";
import reordering_icon from "./reordering_icon.png";
import white_cross_icon from "./white_cross_icon.png";
import agent_icon from "./agent_icon.png";
import your_acts_icon from "./your_acts_icon.png";
import deputy_icon from "./deputy_icon.png";
import placeholder_image from "./placeholder_image.png";
import payment_icon from "./payment_icon.png";
import profile_placeholder from "./profile_placeholder.png";
import bin_icon from "./bin_icon.png";
import Vocalist_Featured_Available from "./Vocalist_Featured_Available.png";
import Deputy_Vocalist_Available from "./Deputy_Vocalist_Available.png";
import security_icon from "./security_icon.png";
import booking_board from "./booking_board.png";
import enquiry_board_icon from "./enquiry_board_icon.png";  
import cover_placeholder from "./cover_placeholder.png";
import moderate_icon from "./moderate_icon.png";
import hero_w_TSC_logo from "./hero_w_TSC_logo.png";
import black_bin_icon from "./black_bin_icon.png";

export const assets = {
  logo,
  add_icon,
  order_icon,
  upload_area,
  parcel_icon,
  ai_icon,
  changing_room_icon,
  hot_meal_icon,
  parking_icon,
enquiry_board_icon,
  roaming_icon,
  cover_overhead_icon,
  dry_and_level_icon,
  cross_icon,
  dropdown_icon,
  info_icon,
  edit_icon,
  white_add_icon,
  white_tick_icon,
  Becky,
  Callum,
  Chris,
  Colin,
  Cyrus,
  DavidK,
  David,
  Emanuele,
  EmmaMay,
  Eve,
  Ewan,
  Frazer,
  Gershom,
  Jeremy,
  Joe,
  Josh,
  Kedesha,
  Kevin,
  Marco,
  Mariam,
  Nicki,
  Phoebe,
  Renes,
  Rya,
  Sam,
  Shamyra,
  Sophie,
  Torann,
  Will,
  reordering_icon,
  white_cross_icon,
  agent_icon,
  your_acts_icon,
  deputy_icon,
  placeholder_image,
  payment_icon,
  profile_placeholder,
  bin_icon,
  Vocalist_Featured_Available,
  Deputy_Vocalist_Available,
  security_icon,
  booking_board,
  cover_placeholder,
  moderate_icon,
  hero_w_TSC_logo,
  black_bin_icon
};



export default assets;
