  ...
  const onSubmit = async (e) => {
    ...
    if (!emailFromUrl || !tokenFromUrl) {
      toast(
        <CustomToast
          type="error"
          message="This reset link is missing details. Please request a new one."
        />
      );
      return;
    }
    ...
    try {
      ...
      const endpoint = `${backendUrl}/api/musician-login/reset-password`;
      ...
      toast(<CustomToast type="success" message="Password reset successfully. Please sign in." />);
      navigate("/login");
    } catch (err) {
    ...
    <>
      ...
      >
        ...
        <p className="text-sm text-gray-500 mt-2 text-center">
          After resetting, you’ll be taken to the login page.
        </p>
      </form>
    </>
  );
